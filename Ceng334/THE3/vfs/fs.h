...
struct file_system_type { //This structure keeps the file system information including the file descriptor to the file system image (opened by the homework shell).
...
struct dentry { //This structure holds information about a directory entry. The directory entries are variable in length since the space needed for the name field size can change. Dentry structure use null
                //terminated strings in the name field.
  unsigned int d_flags;       /* Permissions */
  struct inode *d_inode;      /* Where the name belongs to - NULL is
                               * negative */
  struct dentry *d_parent;    /* parent directory */ //?
  char *d_name;               /* Name */
  struct super_block *d_sb;   /* Pointer to superblock */
  void *d_fsdata;             /* fs-specific data you can use this however
                               * you see fit */
  /* unsigned long d_time; */ /* Deletion time (not important for hw) */
};

struct file_operations {//This structure holds the function pointers to the concrete file system implementations. You will implement these functions in ext2.c and then assign the f_op field every
             //time a file struct is created such that a call like f->f_op->read(...) will execute the function you created in ext2.c.
...
//This functions initializes the ext2 file system by calling initialize_ext2 and gets a reference to its superblock by calling file system type’s get_superblock
//function. It uses the return values of these functions to set the pointers current_fs and current_sb in VFS to store the file system type and superblock. Returns zero for successful operation.You won’t
...
/* Function to close a file, do cleanup here */ //Close file demek herhalde descriptor tablei silcez ve ref_countı, eger ref count 0sa onu kapatcaz filan heralde
