...
public:
    ...
    public:
        ...
        void setOwner(Monitor *o){
            owner = o;
        }
        void wait() {  pthread_cond_wait(&cond, &owner->mut);}
        void notify() { pthread_cond_signal(&cond);}
        void timedWait(long time2Sleep) {
            struct timespec timeStruct;
            clock_gettime(CLOCK_REALTIME, &timeStruct);
            timeStruct.tv_nsec += 1000*time2Sleep;
            timeStruct.tv_sec += (timeStruct.tv_nsec / 1000000000);
            timeStruct.tv_nsec %= 1000000000;
            pthread_cond_timedwait(&cond,&owner->mut,&timeStruct);
