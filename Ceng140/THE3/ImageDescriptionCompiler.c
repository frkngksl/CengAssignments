...
Boolean saveImage ( Image image , char * fileName )
{
  ...
  while(i<pixelNumber){
    if(count == 4){
      fprintf(outputFile,"%3d %3d %3d\n",image.pixels[i].red,image.pixels[i].green,image.pixels[i].blue);
      i++;
      count = 0;
      continue;
    }
    fprintf(outputFile,"%3d %3d %3d ",image.pixels[i].red,image.pixels[i].green,image.pixels[i].blue);
    count++;
    i++;
  }
  fclose(outputFile);
...
)
{
  /* TODO */
  /* ...  */
  Coordinate centerRow = center.row;
  Coordinate centerCol = center.col;
  Coordinate firstx,secondx,firsty,secondy,thirthx,thirthy;
  Point firstPoint,secondPoint,thirthPoint;
  double angle;
  int i=2;
  startAngleDegrees = PI*(startAngleDegrees/180);
  endAngleDegrees = PI*(endAngleDegrees/180);
  angle = (endAngleDegrees-startAngleDegrees)/numberOfSegments;
  firstx=centerRow-(radius*cos(startAngleDegrees));
  firsty=centerCol+(radius*sin(startAngleDegrees));
  firstPoint = makePoint(firstx,firsty);
  secondx = centerRow-(radius*cos(startAngleDegrees+angle));
  secondy = centerCol+(radius*sin(startAngleDegrees+angle));
  secondPoint = makePoint(secondx,secondy);
  drawLine(image,firstPoint,secondPoint,color);
  while(i<numberOfSegments+1){
    thirthx = centerRow - (radius*cos(startAngleDegrees+(angle*i)));
    thirthy = centerCol + (radius*sin(startAngleDegrees+(angle*i)));
    thirthPoint  = makePoint(thirthx,thirthy);
    drawLine(image,secondPoint,thirthPoint,color);
    secondPoint = thirthPoint;
    i++;
  }
  return TRUE;
...
Boolean drawRectangle ( Image image , Point topLeft , unsigned short int width , unsigned short int height , Color color )
{
  /* TODO */
  /* ...  */
  Point topRight,botLeft,botRight;
  Coordinate topLeftCol = topLeft.col;
  Coordinate topLeftRow = topLeft.row;
  if(width == 0){
    return FALSE;
  }
  if(height == 0){
  ...
  topRight = makePoint(topLeftRow,(topLeftCol+(width-1)));
  botLeft = makePoint((topLeftRow+(height-1)),topLeftCol);
  botRight = makePoint((topLeftRow+(height-1)),(topLeftCol+(width-1)));
  drawLine(image,topLeft,topRight,color);
  drawLine(image,topRight,botRight,color);
  drawLine(image,botRight,botLeft,color);
  drawLine(image,botLeft,topLeft,color);
  return TRUE;
}
...
Boolean drawRegularConvexPolygon ( Image image , Point center , unsigned short int radius , unsigned short int numberOfSides , Color color )
{
  ...
  Coordinate firstx,secondx,firsty,secondy,thirthx,thirthy;
  Point firstPoint,secondPoint,thirthPoint;
  double angle = ((2*PI)/numberOfSides);
  int i=2;
  firstx=centerRow-radius;
  firsty=centerCol;
  firstPoint = makePoint(firstx,firsty);
  secondx = centerRow-(radius*cos(angle));
  secondy = centerCol+(radius*sin(angle));
  secondPoint = makePoint(secondx,secondy);
  drawLine(image,firstPoint,secondPoint,color);
  while(i<numberOfSides){
    thirthx = centerRow - (radius*cos(angle*i));
    thirthy = centerCol + (radius*sin(angle*i));
    thirthPoint  = makePoint(thirthx,thirthy);
    drawLine(image,secondPoint,thirthPoint,color);
    secondPoint = thirthPoint;
    i++;
  }
  drawLine(image,thirthPoint,firstPoint,color);
  return TRUE; }
}
...
Boolean drawCharacter ( Image image , Point topLeft , char c , Color color )
{
  ...
  int asciiValue = c;
  int col,row;
  Coordinate firstTopLeft = topLeft.col; /* topLeft' first column's coordinate */
  row = asciiValue-32; /* position in characterRasters*/
  for(col=12;col>=0;col--,topLeft.row++){
      unsigned char checkingBinaryValue = characterRasters[row][col];
      int i=0;
      topLeft.col = firstTopLeft + 8*sizeof(unsigned char);
      while(i<8){
      if(checkingBinaryValue%2 == 1){
        /* If a binary number has 1 in the last digit ie. 0000 0011 = 3, it is an odd number. */
        setPixel(image,topLeft,color);
        topLeft.col= topLeft.col - sizeof(unsigned char );
        i++;
        checkingBinaryValue = checkingBinaryValue >> 1;
      }
      else{
      topLeft.col= topLeft.col - sizeof(unsigned char);
      i++;
